...
func (vu *VideoUpload) Upload(objectPath string, client *storage.Client, ctx context.Context) error {
...
func (vu *VideoUpload) ProcessUpload(concurrency int, doneUpload chan string) error {
	in := make(chan int, runtime.NumCPU()) //qual arquivo vai ler baseado pa posição
...
func (vu *VideoUpload) uploadWorker(in chan int, returnChan chan string, uploadClient *storage.Client, ctx context.Context) {
	for x := range in {
		err := vu.Upload(vu.Paths[x], uploadClient, ctx)
		if err != nil {
