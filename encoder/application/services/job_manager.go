...
func (j *JobManager) Start(ch *amqp.Channel) {
	...
	if err != nil {
		log.Fatalf("errorl oading var: CONCURRENCY_WORKERS")
	}
...
func (j *JobManager) checkParseErrors(jobResult JobWorkerResult) error {
	if jobResult.Job.ID != "" {
		log.Printf("Message ID: %v. eRROR DURING THE JOB: %v WITH VIDEO: %v. Errror %v", jobResult.Message.DeliveryTag, jobResult.Job.ID, jobResult.Job.Video.ID, jobResult.Error.Error())
	} else {
