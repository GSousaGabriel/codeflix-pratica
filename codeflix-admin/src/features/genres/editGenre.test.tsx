...
const handlers = [
  http.get(`${baseUrl}/genres/undefined`, async () => {
    await delay(150)
    return HttpResponse.json({data: mockData}, { status: 200 });
  }),
  http.get(`${baseUrl}/categories`, async () => {
    await delay(150)
    return HttpResponse.json(categoryResponse, { status: 200 });
  }),
  http.post(`${baseUrl}/genres/1`, async () => {
    await delay(150)
...
describe("editGenre", () => {
  ...
  it("should validate and save on submit", async () => {
    ...
    await vi.waitFor(() => {
      const message = screen.getByText("Genre updated successfully");
  ...
  it("should give error on submit", async () => {
    ...
    await vi.waitFor(() => {
      const message = screen.getByText("Genre not updated");
      expect(message).toBeInTheDocument();
    });
  });
});
